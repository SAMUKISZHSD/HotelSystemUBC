...
public class HospedeDAO {
    ...
    public void addHospede(Hospede hospede) {
        try (Connection connection = DatabaseConnection.getConnection()) {
            String query = "INSERT INTO Hospedes (id_hospedes, nome_completo, dt_nascimento, endereço, telefone, email, cpf) VALUES (?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement stmt = connection.prepareStatement(query);
    ...
    public void updateHospede(Hospede hospede) {
        try (Connection connection = DatabaseConnection.getConnection()) {
            String query = "UPDATE Hospedes SET nome_completo = ?, dt_nascimento = ?, endereço = ?, telefone = ?, email = ?, cpf = ? WHERE id_hospedes = ?";
            PreparedStatement stmt = connection.prepareStatement(query);
